...
- Releasing before 36 points closes the row. Releasing from 36 points onward settles at a 44-point stop that exactly reveals a 44×24 `HIDE` button matching `COPY`. Its first visible edge is circular, then settles to the standard row-button shape. Pulling past 44 points continuously expands the darker `#c80f28` action until it fills the row at the 55% commit threshold; hiding still occurs only after release, and a 16-point retreat hysteresis cancels that full-swipe state.
