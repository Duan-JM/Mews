...
New completion and failure transitions pulse red for two seconds on either the collapsed notch contour or the expanded shell edge. Expanding during a pulse preserves its remaining duration, and a stop received while expanded starts the same bounded edge pulse without collapsing the shell. Startup history is synchronized silently, so relaunching Mews does not replay stale stop pulses. After the pulse, aggregate session state restores green when another session is running or retains steady red when every displayable session has stopped. `needs_input` keeps a red breathing glow until resolved across collapsed and expanded states. Transition pulses are deduplicated by the presentation transition identifier. `running` stays green, while `idle` orders the collapsed overlay out.
