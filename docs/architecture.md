...
AppKit keeps the content and input panel at 420×220 points. In physical-notch mode, a passive child window below it reserves 32 points beside and below the shell for the glow. It always ignores mouse events, carries no independent accessibility surface, and follows the same snapshot, display placement, visibility, and teardown as the content panel. Top-center mode keeps that child window hidden. Hiding detaches the child in AppKit, so showing reattaches it explicitly. Both windows register to the content panel's actual AppKit frame. The backing keeps a stable layout container across content changes; replacing a `Group` child must not snap one surface to its final size. One bounded main-run-loop clock samples spring geometry from monotonic time and publishes each layout to both hosting windows, without independent SwiftUI geometry interpolation. Sharing spring parameters or a SwiftUI transaction alone does not synchronize separate hosting clocks. The shared spring retains its 0.3-second response, 0.88 damping ratio, and velocity when interrupted; it stops on completion, hiding, or Reduce Motion. Content opacity is scoped inside the frame and cannot animate its geometry. Only content is clipped to the moving shell so fading text cannot escape; the backing and glow remain outside that clip. Native motion regressions sample left, right, and bottom edges throughout expansion, collapse, and interrupted breathing transitions at 1×/2×, including content containment and enabling Reduce Motion during motion. Hit testing includes the software backing and solid edge but excludes the halo. The physical notch remains a trigger when the idle overlay is hidden. Outside clicks close the panel without consuming or synthesizing the target event. The fallback uses adaptive material with a native window shadow and sits six points below the visible menu-bar edge. Left-clicking the status item toggles the shell; right-click and Control-click retain the menu. If global hover monitoring is unavailable, Mews logs the degradation and keeps status-item click and top-center fallback available.

Motion rendering tests advance an injected clock between captures so a slow rasterizer cannot skip the intermediate geometry being checked. A deliberately stalled 2× capture preserves the same 1px edge limit and positive intermediate-frame assertion. Separate tests exercise the real timer's frame publication and cancellation, and compare the spring sampler against SwiftUI's spring values, including reversal velocity.
