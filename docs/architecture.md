...
The shell keeps a pure `closed` / `peek` / `expanded` interaction policy separate from AppKit timers and event monitors. On a physical notch, `closed` renders no surface while idle, a steady green edge glow while any session runs, a breathing red glow while input is needed, or a steady red glow when every displayable session has stopped. The collapsed shape adds four points on each horizontal side but no vertical padding: its height equals the measured notch safe-area height. A top-aligned mask clips the composited stroke and shadows to that height, outside the animated shell layout, so the glow cannot spill above or below the menu-bar band. `peek` uses that same geometry with a two-second red breathing pulse for a newly stopped session. `expanded` keeps the existing full black panel and inherits the current glow around its outer shell, including an active pulse without restarting its timer. The top-center fallback still renders only `expanded`. AppKit owns the fixed 420×220 nonactivating panel, display placement, passive local/global mouse observation, and teardown. The physical notch remains an interaction trigger even when the idle overlay is ordered out. Hit testing derives from the rendered glow or expanded shell rather than the transparent maximum panel. Outside clicks close an expanded panel without consuming or synthesizing the target event. The fallback uses rounded adaptive material with a native window shadow and sits six points below the visible menu-bar edge. Left-clicking the status item toggles the shell, while right-click and Control-click preserve the existing event, Refresh, and Quit menu. Physical-notch hover is optional: if global hover monitoring is unavailable, the app logs the degradation and keeps the status-item click and top-center fallback paths.
