...
- Open the expanded status shell at the physical notch or top center from that pixel logo.
...
- Route each attention event to the physical-notch signal or a macOS notification fallback.
...
The agent is packaged as a small app bundle so macOS menu bar identity, notification permission, and local visibility are reliable. It starts the bundled `mw agent` helper, renders a state-responsive template pixel logo, reads local session and event state for the context menu, renders a collapsed physical-notch glow, opens the notch/top-center shell on demand, and routes attention events through either the physical-notch signal or native notifications. Before launching a child and after a child exits, the app asynchronously pings the same Unix socket path used by the Go store resolver, including the private short-path and namespace fallback. In-flight probes are deduplicated so the main actor never waits on IPC. A responsive external agent is treated as healthy, is never duplicated, and is not terminated when the app exits. While externally owned, liveness is checked by the existing two-second refresh. A missing helper, launch failure, or short-lived child uses monotonic exponential restart delays from 10 seconds to a five-minute cap. One minute of healthy child runtime resets the delay, which avoids a persistent process-launch loop while retaining automatic recovery.

The shell keeps a pure `closed` / `peek` / `expanded` interaction policy separate from AppKit timers and event monitors. On a physical notch, `closed` renders no surface while idle, a steady green edge glow while any session runs, a breathing red glow while input is needed, or a steady red glow when every displayable session has stopped. macOS supplies a safe-area rectangle, not the exact hardware silhouette. The active collapsed signal therefore uses an opaque black backing derived from the safe-area height, widened by its eight-point corner radius and extended two points on the sides and bottom so the rounded backing covers display-edge antialiasing. This joins the hardware black to a software-controlled boundary without a transparent gap; it does not claim pixel-perfect hardware tracing. A double-width stroke behind the backing leaves a uniform visible outside edge, and its open path has no top segment. One soft shadow fades continuously below the menu bar. `peek` uses the same geometry with a two-second red breathing pulse. `expanded` keeps the full black panel, with the same left/right/bottom glow and an active pulse that does not restart. The top-center fallback renders only `expanded`, with its adaptive or opaque surface and native window shadow but no colored outline, halo, breathing edge, or stop-pulse edge.

AppKit keeps the content and input panel at 420×220 points. In physical-notch mode, a passive child window below it reserves 32 points beside and below the shell for the black backing and glow. It always ignores mouse events, carries no independent accessibility surface, and follows the same snapshot, display placement, visibility, and teardown as the content panel. Top-center mode keeps that child window hidden. Hiding detaches the child in AppKit, so showing reattaches it explicitly. Both windows register to the content panel's actual AppKit frame. The physical-notch content window renders only the expanded interactive content; its geometry does not observe per-frame spring updates. Content appears after expansion reaches its target and hides before collapse starts, so it cannot escape the moving backing. The passive visual window renders backing and glow together with `CAShapeLayer`, avoiding independent SwiftUI hosting commits. SwiftUI paths use top-down view coordinates, so the layer boundary explicitly converts them to Core Animation's bottom-up coordinates around the visual window height; the backing remains attached to the screen top and the lower halo margin stays transparent. On macOS 14 and later, the currently visible shell window's display link samples spring geometry from monotonic time and targets 60 Hz. macOS 13 retains a main-run-loop timer fallback because AppKit display links require macOS 14. The top-center fallback continues to render its complete shell in the content window. The shared spring retains its 0.3-second response, 0.88 damping ratio, and velocity when interrupted; it stops on completion, hiding, or Reduce Motion. Native motion regressions sample left, right, and bottom edges throughout expansion, collapse, and interrupted breathing transitions at 1×/2×, including content containment and enabling Reduce Motion during motion. Hit testing includes the software backing and solid edge but excludes the passive visual window and its halo. The physical notch remains a trigger when the idle overlay is hidden. Outside clicks close the panel without consuming or synthesizing the target event. The fallback uses adaptive material with a native window shadow and sits six points below the visible menu-bar edge. Left-clicking the status item toggles the shell; right-click and Control-click retain the menu. If global hover monitoring is unavailable, Mews logs the degradation and keeps status-item click and top-center fallback available.

Motion rendering tests capture through AppKit's display path and advance an injected clock between captures so a slow rasterizer cannot skip the intermediate geometry being checked. They assert the physical backing starts at the screen top, does not enter the lower halo margin, and keeps its square top and rounded bottom orientation at 1×/2×. A deliberately stalled 2× capture preserves backing/glow alignment within the configured outline width and a positive intermediate-frame assertion. Separate tests exercise the real timer's frame publication and cancellation, and compare the spring sampler against SwiftUI's spring values, including reversal velocity.

Display placement is recalculated on `NSApplication.didChangeScreenParametersNotification`. The resolver prefers any available physical notch, otherwise uses the main display's `visibleFrame` top center so the shell stays below the menu bar. This covers external-display, clamshell, resolution, coordinate, and main-screen changes. A transient empty screen list clears placement, closes the interaction state, and orders the panel out; the next display notification restores the collapsed state without reopening the expanded panel. The panel remains stationary and joins normal Spaces, but does not participate as a full-screen auxiliary window. A separate 100 ms probe reads only the frontmost process identifier plus on-screen window layers and bounds; a full-frame window or full-width top cover on the overlay's display suppresses both shell windows until that display returns to a normal desktop layout.
...
New completion and failure transitions pulse red for two seconds on either the collapsed physical-notch contour or its expanded shell edge. Expanding during a pulse preserves its remaining duration, and a stop received while the notch shell is expanded starts the same bounded edge pulse without collapsing the shell. Top-center placement keeps the semantic transition and notification delivery without drawing that notch-only effect. Startup history is synchronized silently, so relaunching Mews does not replay stale stop pulses. After the pulse, aggregate session state restores green when another session is running or retains steady red when every displayable session has stopped. `needs_input` keeps a red breathing glow until resolved across collapsed and expanded physical-notch states. Transition pulses are deduplicated by the presentation transition identifier. `running` stays green, while `idle` orders the collapsed overlay out.

Presentation selection uses an injected current time rather than mutating stored history. `running` and `needs_input` events remain current for 24 hours; settled `done`, `failed`, and explicit `idle` events remain current for 30 minutes. Events more than five minutes in the future are not selected as current. When the latest primary event expires, the status logo returns to `idle`, transient stop pulses close, and current attention resolves. Expired primary events remain in bounded local history but do not occupy the expanded active-session panel; events without stable identity keep their separate bounded menu entries.
...
The status item uses a monochrome template image so system menu bar contrast remains authoritative. `NSWorkspace.accessibilityDisplayOptionsDidChangeNotification` refreshes panel rendering when Reduce Motion, Reduce Transparency, or Increase Contrast changes. Reduce Motion keeps the red or green signal but removes breathing and spatial shell transitions. Reduce Transparency and Increase Contrast replace top-center material with an opaque appearance-aware surface; Increase Contrast also strengthens the notch outline and raises secondary-copy, separator, border, status, and disabled-control contrast. The expanded physical-notch shell remains solid black. The status item, panel window, shell container, summaries, and buttons expose accessibility labels or native text for VoiceOver; these adaptations do not add a Mews permission prompt.
...
- Primary-agent `needs_input`: keep a red breathing physical-notch glow until resolved, or notify immediately when no physical notch is available.
- Primary-agent non-recoverable `failed`: pulse red for two seconds, then derive green or steady red from the remaining displayable sessions, or notify immediately when no physical notch is available.
- Primary-agent `done`: pulse red for two seconds, then derive green or steady red from the remaining displayable sessions, or notify immediately when no physical notch is available.
...
13. The pixel logo opens the notch/top-center panel, while right-click and Control-click retain the existing menu.
...
15. A physical notch stays visually quiet while idle, uses aggregate red or green edge glows that hug the hardware while collapsed, and carries the same effect onto the existing expanded panel.
...
- Closed, stop-pulse, and expanded shell policy, aggregate glow state, pulse timing, deduplication, rendered-shell hit testing, and physical-notch geometry.
