...
The shell keeps a pure `closed` / `peek` / `expanded` interaction policy separate from AppKit timers and event monitors. On a physical notch, `closed` renders no surface while idle, a steady green edge glow while any session runs, a breathing red glow while input is needed, or a steady red glow when every displayable session has stopped. The collapsed contour uses the measured notch width and safe-area height without extra padding. Its stroke is centered on that contour at twice the intended visible width: hardware occludes the inner half, leaving a uniform outside edge along both straight sides and curves. No second software cutout or menu-band mask truncates its antialiased pixels. Soft shadows originate from the same contour and may fade beyond the menu bar. The OS supplies a notch bounding region, not its exact corner curve; the existing eight-point quadratic corner remains a hardware approximation. `peek` uses that same geometry with a two-second red breathing pulse for a newly stopped session. `expanded` keeps the existing full black panel and inherits the current glow around its outer shell, including an active pulse without restarting its timer. The top-center fallback still renders only `expanded`. AppKit owns the fixed 420×220 nonactivating panel, display placement, passive local/global mouse observation, and teardown. The physical notch remains an interaction trigger even when the idle overlay is ordered out. Collapsed hit testing includes the notch and its visible solid outline, excluding the soft halo and transparent maximum panel; expanded hit testing follows the full shell. Outside clicks close an expanded panel without consuming or synthesizing the target event. The fallback uses rounded adaptive material with a native window shadow and sits six points below the visible menu-bar edge. Left-clicking the status item toggles the shell, while right-click and Control-click preserve the existing event, Refresh, and Quit menu. Physical-notch hover is optional: if global hover monitoring is unavailable, the app logs the degradation and keeps the status-item click and top-center fallback paths.
