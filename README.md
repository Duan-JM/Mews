...
- A menu bar companion with a state-responsive pixel logo, a prioritized recent-session list, and a compact notch/top-center shell.
...
The expanded shell shows at most three sessions, ordered by `needs_input`, `failed`, unacknowledged `done`, `running`, then acknowledged or idle recent sessions. Each row shows the agent, bounded project label, shortened session reference, status, and its own **Return** and **Copy** controls. Row order and health controls freeze while the panel is expanded so a refresh cannot move the target beneath the pointer. Events without a stable session identity remain bounded history instead of becoming fake sessions. Full session identifiers, working directories, subagent events, recoverable failures, prompt text, and runner command text are not rendered. Actions use only validated local context and Mews-generated history commands; unavailable actions stay disabled.
